...
import CellMergeCellsOverlay from '$lib/components/editable-grid/interactions/cell-merge-split-drag/cell-merge-cells-overlay.svelte';
...
> {
	...
	onCellMouseMove({ cell, originalEvent }: DataTypeOf<typeof gridEvents.cell.mouseMove>) {
		...
		const axisAlignedDistance = Math.max(Math.abs(clientX - startX), Math.abs(clientY - startY));
